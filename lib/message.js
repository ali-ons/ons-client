...
function Message(options) {
  ...
  if (options.tag) {
    this.systemProperties.set(TAG, options.tag);
  }
  if (options.key) {
    this.systemProperties.set(KEY, options.key);
  }
}
...
proto.getTag = function() {
  return this.systemProperties.get(TAG);
};

proto.setTag = function(tag) {
  assert(tag && util.isString(tag));
  this.systemProperties.set(TAG, tag);
  return this;
};
...
proto.setKey = function(key) {
  ...
  this.systemProperties.set(KEY, key);
  return this;
};

proto.getMsgID = function() {
  return this.systemProperties.get(MSGID);
};

proto.setMsgID = function(msgid) {
  assert(msgid && util.isString(msgid));
  this.systemProperties.set(MSGID, msgid);
  return this;
};
